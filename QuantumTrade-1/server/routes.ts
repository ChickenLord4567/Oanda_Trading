...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/chart/:instrument/:timeframe', requireAuth, async (req, res) => {
    try {
  ...
  app.post('/api/cleanup-trades', requireAuth, async (req, res) => {
    try {
      ...
      if (orphanedTrades.length > 0) {
        ...
        console.log(`🧹 Cleaned up ${deletedCount} orphaned trades:`, orphanedIds);
      }
      ...
      if (missingSyncTrades.length > 0) {
        for (const oandaTrade of missingSyncTrades) {
          try {
            const tradeData = {
              ...
              direction: parseFloat(oandaTrade.currentUnits) > 0 ? 'buy' as const : 'sell' as const,
        ...
        console.log(`🔄 Synced ${syncedCount} existing OANDA trades to MongoDB`);
      }
